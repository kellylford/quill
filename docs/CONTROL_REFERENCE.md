<!-- AUTO-GENERATED FILE. Do not hand-edit.
    Generated by quill/tools/build_docs.py on 2026-06-15 04:43 UTC.
...
Lists all built-in and custom image description styles. Arrow through to hear each style's name. Disabled built-in styles show [hidden] before the name. Select a style to read its full prompt text in the preview pane below.

| Key | Action |
|---|---|
| Alt+D | Disable/Enable selected |
| Alt+A | Add custom prompt |
| Alt+E | Edit custom prompt |

### Prompt Text Preview

Shows the full prompt text for the selected image description style. This is the instruction sent to the AI model when describing an image. Built-in prompt text is read-only; custom prompts can be edited.
